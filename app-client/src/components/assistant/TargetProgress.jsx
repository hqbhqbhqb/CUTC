...
function TargetProgress({ targets = [], activeTargetId }) {
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center justify-between"><h3 className="font-bold">Target progress</h3><span className="text-sm font-semibold text-[#2d7a6d]">{completed} / {targets.length}</span></div>
      <div className="mt-5 grid grid-cols-2 gap-3">
        ...
        {targets.length === 0 && <p className="col-span-2 py-3 text-center text-sm text-[#82928e]">No targets detected.</p>}
      </div>
    </Card>
  );
}
